...
const handleDelete=()=>{
  ...
  <form onSubmit={handleSave} className='bg-white p-6 rounded-lg w-full max-w-2xl'>
<h2 className='text-lg font-semibold shadow-lg w-full. max-w-2xl'>Edit Task</h2>
<label className='block text-sm mb-1'>Title</label>
<input value={title} onChange={(e)=>setTitle(e.target.value)} className='w-full p-2 border rounded mb-3' />

<label className='block text-sm mb-1'>Description</label>
<textarea value={description} onChange={(e)=>setDescription(e.target.value)} rows={4} className='w-full p-2 border rounded mb-3' />
<div className='grid grid-cols-3 gap-3 mb-3'>
<div>
  <label className='text-sm block mb-1'> Priority</label>
  <select value={priority} onChange={(e)=>setPriority(e.target.value)} className='w-full p-2 rounded border'>
...
<input value={tagsText} onChange={(e)=>setTagsText(e.target.value)} placeholder='tag1,  tag2' className='w-full p-2 border rounded' />
</div>
</div>

<div className="flex justify-between items-center mt-4">
          <div className="flex gap-2">
            <button type="submit"   className="px-3 py-2 cursor-pointer bg-blue-600 text-white rounded">Save</button>
          </div>
